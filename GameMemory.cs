...
namespace LiveSplit.RedFaction
{
	class GameMemory
	{
		...
		MemoryWatcherList _WatchersList;
		private DeepPointer _levelNamePtr;
		private MemoryWatcher<bool> _isLoadingWatcher;
		private MemoryWatcher<bool> _binkMoviePlaying;
		private MemoryWatcher<int> _gameState;
		...
		public GameMemory(RedFactionSettings componentSettings)
		{
			...
			_WatchersList = new MemoryWatcherList();
			_isLoadingWatcher = new MemoryWatcher<bool>(new DeepPointer(0x13756AC)); // == 1 if a loadscreen is happening
			_levelNamePtr = new DeepPointer(0x0246144, 0x0);
			_binkMoviePlaying = new MemoryWatcher<bool>(new DeepPointer("binkw32.dll", 0x41BD8));    //binkw32.dll+41BD8
			_gameState = new MemoryWatcher<int>(new DeepPointer(0x230064));

			_WatchersList.Add(_isLoadingWatcher);
			_WatchersList.Add(_binkMoviePlaying);
			_WatchersList.Add(_gameState);
			...
			while (!_cancelSource.IsCancellationRequested)
			{
				try
				{
					...
					while (!game.HasExited)
					{
						...
						if (streamGroupId != prevStreamGroupId && streamGroupId != null || _binkMoviePlaying.Changed)
						{
							...
							else if (streamGroupId == LevelName.Bomb && _binkMoviePlaying.Current)
							{
								Split(SplitArea.Bomb, frameCounter);
							}
						}

						if (_isLoadingWatcher.Changed || _binkMoviePlaying.Changed || _gameState.Changed)
						{
							if (_isLoadingWatcher.Current)
							{
								Debug.WriteLine(String.Format("[NoLoads] Load Start - {0}", frameCounter));

								loadingStarted = true;

								if(_gameState.Current != 5 || !_settings.DontPauseOnSaveLoads)
								{
									// pause game timer
									_uiThread.Post(d =>
									{
								...
								if (streamGroupId == LevelName.Chapter1Start && _binkMoviePlaying.Current)
								{
									//reset game timer
									_uiThread.Post(d =>
									{
